...
class AdminUser extends MY_Controller {
	...
	public function index()	{
		if( $this->Admin_model->getAdminType() == 1 ) {
            ...
            $arrSendToView['page_title'] = "Admin User";
            $arrSendToView['SegmentTitle'] = "Admin User Form";
    ...
    public function ajaxAddUser() {
        ...
        if( $this->Admin_model->getAdminType() == 1 ) {
            ...
            $recordByMail = $this->Utility_model->returnRecord("admin_user", false, array("Email"=>$arrFrInsert['Email']));

            //printArr($recordByMail);

            if( $recordByMail == false ) {
                $arrFrInsert['RegistrationDate'] = dateForDb( time() );
                $arrFrInsert['UpdateTime'] = dateForDb( time() );

                //printArr($arrFrInsert);
                //die();

                $newInsertedId = $this->Utility_model->insertRecort('admin_user', $arrFrInsert );
                ...
                $tmpRespondArr['error_flag'] = TRUE;
                $tmpRespondArr['error_msg'] = "AlreadyExist";
            }
    ...
    public function ajaxUserList() {
        if( $this->Admin_model->getAdminType() == 1 ) {
            $data['recordList'] = $this->Utility_model->returnRecord("admin_user", false, array("UserType"=>1));
            $returnRespond = array("respond"=>array("error_flag"=>false, "content"=>$this->load->view('User/user_list', $data, true)));
            echo json_encode($returnRespond);
        } else {
            session_destroy();
            header("Location: ".base_url());
    ...
    public function ajaxDeleteUser() {
        ...
        if( $this->Admin_model->getAdminType() == 1 ) {
            if( isset($_POST['id']) && trim($_POST['id']) != "" ) {
                $chkFlag = $this->Utility_model->returnRecord("admin_user", false, array("PK_AdminUserId"=>$_POST['id']));

                if( $chkFlag == FALSE ) {
                    $returnRespond = array("respond"=>array("error_flag"=>true, "error_msg"=>"ErrorDoesNotExist"));
                } else {
                    $this->db->delete('admin_user', array("PK_AdminUserId"=>$_POST['id']));
                ...
                $returnRespond = array("respond"=>array("error_flag"=>true, "error_msg"=>"ErrorIdMissing"));
            }
        } else {
            $returnRespond = array("respond"=>array("error_flag"=>true, "error_msg"=>"not_logged_in"));
        }
