...
function addUser() {
    ...
    else if( $( "#Re_Password" ).val().trim() == "" ) {
        alert("Re-Password Must Be Provided");
        $( "#Re_Password").focus();
        isValid = false;
    }
    else if( $( "#Password" ).val().trim() != $( "#Re_Password" ).val().trim() ) {
        ...
        $.post( "AdminUser/ajaxAddUser",
            {
                ...
                if( ajaxRespond.respond.error_flag ) {
                    ...
                    else if( ajaxRespond.respond.error_msg == "AlreadyExist" ) {
                        $( "#error_msg_div").show("slow");
                        $( "#error_msg").html("E-Mail already used.");
                    }
...
function loadUserList() {
    load_ajax_loader("divList", "40px", "48.5%");

    setTimeout(function(){
        $.post( "AdminUser/ajaxUserList",
            function( data ) {
                var ajaxRespond = JSON.parse(data);
                ...
                if( ajaxRespond.respond.error_flag ) {
                    ...
                    if(typeof ajaxRespond.respond.content != 'undefined' && ajaxRespond.respond.content != "" ) {
                        $("#divList").html(ajaxRespond.respond.content).promise().done(function() {
        ...
        $.post( "AdminUser/ajaxDeleteUser",
            ...
            function( data ) {
                ...
                if( responseObj.respond.error_flag ) {
                    ...
                    else if( responseObj.respond.error_msg == "ErrorIdMissing" ) {
                        $( "#error_msg_div").show("slow");
